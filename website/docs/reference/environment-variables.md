...
| `AGENT_BROWSER_FORCE_SANDBOX` | Legacy/deployment fallback used only when `browser.force_sandbox` is absent. Configure normal behavior with `hermes config set browser.force_sandbox true`, which requires a non-root Linux user and working user namespaces and fails closed instead of retrying without the sandbox. It rejects `--no-sandbox`, `--no-zygote-sandbox`, `--disable-setuid-sandbox`, `--disable-namespace-sandbox`, `--disable-gpu-sandbox`, `--disable-seccomp-filter-sandbox`, `--single-process`, and `--in-process-gpu` in `AGENT_BROWSER_ARGS` or legacy `AGENT_BROWSER_CHROME_FLAGS`. An explicit config value, including `false`, overrides this variable. On non-Linux systems, enabled local browser commands fail with an error instead of launching. Do not add this behavioral setting to `.env` for new configurations. |
